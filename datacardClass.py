...
class datacardClass:
    ...
    def theInputProp(self, theInputs):
        _lumi = theInputs['lumi']
        _sqrts = theInputs['sqrts']
        _channel = theInputs['decayChannel']
        _all_chan = theInputs['all']
        _ggH_chan = theInputs['ggH']
        _qqH_chan = theInputs['qqH']
        _WH_chan = theInputs['WH']
        _ZH_chan = theInputs['ZH']
        _ttH_chan = theInputs['ttH']
        _qqZZ_chan = theInputs['qqZZ']
        _ggZZ_chan = theInputs['ggZZ']
        _zjets_chan = theInputs['zjets']
        _ttbar_chan = theInputs['ttbar']

        print('Inputs can be loaded!')
    ...
    def makeCardsWorkspaces(self, theMH, theis2D, theOutputDir, theInputs,theTemplateDir="templates2D", theIncludingError=False, theVBF=False, theVBFcat=0, theREFIT=False):
        ...
        CMS_zz4l_mean_e_err.setConstant(ROOT.kTRUE)
        CMS_zz4l_sigma_e_sig.setVal(0)
        CMS_zz4l_mean_m_sig.setVal(0)
        CMS_zz4l_mean_m_err.setConstant(ROOT.kTRUE)
        ...
        # For the moment we only run with bVBF = False option
        # So let's drop all the cases where there are if realted to it
        # if not self.bVBF:
        sigWH = WH
        sigZH = ZH
        ...
        nonresWH = ROOT.RooChebychev(name,name,CMS_zz4l_mass, ROOT.RooArgList(rfv_WHp0,rfv_WHp1) )
        ...
        nonresZH = ROOT.RooChebychev(name,name,CMS_zz4l_mass, ROOT.RooArgList(rfv_ZHp0,rfv_ZHp1) )
  ...
  #------------------------------------------------end bIncludingError
        ...
        bkg_qqzz = ROOT.RooChebychev(name,name,CMS_zz4l_mass,ROOT.RooArgList(qqzz_c0,qqzz_c1,qqzz_c2))
        ...
        bkg_ggzz = ROOT.RooChebychev(name,name,CMS_zz4l_mass,ROOT.RooArgList(ggzz_c0,ggzz_c1,ggzz_c2))

        ## Reducible backgrounds
        bkg_zjets = ROOT.RooGenericPdf();
        print "zjets mass shape"

        # For the moment we run w/o refit, so I remove everything related to it
        # if not self.isREFIT:
        if (self.channel == self.ID_4mu):
            p0_zjets_4mu = ROOT.RooRealVar("p0_zjets_4mu","p0_zjets_4mu",134.6)
            p1_zjets_4mu = ROOT.RooRealVar("p1_zjets_4mu","p1_zjets_4mu",24.4)
            landau_zjets_4mu = ROOT.RooFormulaVar("landau_zjets_4mu","TMath::Landau(@0,@1,@2)",ROOT.RooArgList(CMS_zz4l_mass,p0_zjets_4mu,p1_zjets_4mu))
        ...
        if (self.channel == self.ID_2e2mu):
            ...
            landau_zjets_2e2mu = ROOT.RooFormulaVar("landau_zjets_2e2mu","TMath::Landau(@0,@1,@2)",ROOT.RooArgList(CMS_zz4l_mass,p0_zjets_2e2mu,p1_zjets_2e2mu))
      ...
      ## ----------------- 2D BACKGROUND SHAPES --------------- ##
        ...
        tagRatio_qqzz = ROOT.RooRealVar(name,name,bkgRate_qqzz_VBFtagged/bkgRate_qqzz_untagged)
        name = "tagRatio_ggzz_{0:.0f}".format(self.channel)
        tagRatio_ggzz = ROOT.RooRealVar(name,name,bkgRate_ggzz_VBFtagged/bkgRate_ggzz_untagged)     
